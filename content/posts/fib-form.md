...
Since $\det A=-1$, $A$ is invertible, and its inverse must be such that
$$\begin{pmatrix}f_{n}\\ f_{n-1}\end{pmatrix} = A^{-1} \begin{pmatrix}f_{n+1}\\ f_{n}\end{pmatrix} $$
for all $n\ge 0$.
{{< remark >}}
We can find $A^{-1}$ with a formula[^1], but one can avoid that for a more conceptual approach. Observe for all $n\in\mathbb Z$
$$\begin{pmatrix}f_{n}\\ f_{n-1}\end{pmatrix} = \begin{pmatrix} 0 & 1 \\ 1 & -1 \end{pmatrix} \begin{pmatrix}f_{n+1}\\ f_{n}\end{pmatrix} $$
This reverses what $A$ does, so we expect $B$ to be the inverse of $A$ where 
...
Therefore equations (1) and (2) holds hold for all $n\in\mathbb Z$, and hence equation (3) hold for all $n\in\mathbb Z$ as well. This immediately gives the following consequence
...
{{< theorem >}}
If $n\mid m$ then $F_n\mid F_m$
{{< /theorem >}}

[^1]: For any invertible $2$-by-$2$ matrix $A$ we have $A^{-1}=\frac{1}{ad-bc}\begin{pmatrix}d & -b \\ -c & a \end{pmatrix}$ where $A=\begin{pmatrix}a & b \\ c & d \end{pmatrix}$. 